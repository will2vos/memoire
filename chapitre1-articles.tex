...
\begin{figure}[ht!]
	\centering
	\includegraphics[scale=0.70]{fig_sites.png}
...
Using SEM allow us to measure direct and indirect processes that influence species occurrence by considering interrelationships between treatments, taxa and environmental variables (Figure \ref*{fig:SEM}, \citealp{graceSpecificationStructuralEquation2010}).
\\

\begin{figure}[ht!]
	\centering
	\includegraphics[scale=0.65]{fig_sem3.png}
...
Ocupancy models allow us to account for imperfect detection of species since salamanders and carabids are known to be cryptic species and therefore may be present but unobserved \citep{baileyEstimatingSiteOccupancy2004,spiersEstimatingSpeciesMisclassification2022}
...
%In our models, we used overstory treatments to explain the presence of salamanders and both carabids groups while considering precipitation levels, CWD and a block random effect to estimate detection probability.

We calculated occupancy probabilities for salamanders and carabids form the salamanders competitor group at a site $i$ at a time $j$ with a Bernoulli distribution where the probability of success were estimated from the latent occupancy state $z$ and the detection $p$.
...
We used a Normal distribution to estimate posteriors and we included overstory treatments (i.e. Cutpartial, Cutclear) and a block random effect in linear predictors to estimate variables means. 
...
% These approaches combine two regression components in a single hierarchical model: one component estimates the state
% variable (abundance: λ, occupancy: ѱ) and the effect of explanatory variables influencing the
% state variable, whereas the second component estimates the detection probability (p) and the
% effect of variables influencing detection probability (Mazerolle et al., 2007; Royle & Dorazio,
% 2008)

% We conducted model
% selection and inference based on the second-order Akaike information criterion (Burnham
% and Anderson, 2002; Mazerolle, 2006). Model selection and multimodel inference using the
% shrinkage estimator were implemented with the AICcmodavg package (Burnham &
% Anderson, 2002; Mazerolle, 2015, 2020). For site occupancy models, we assessed the fit with
% the MacKenzie & Bailey (2004) goodness-of-fit of fit test.

% All parameters in our analyses were estimated by maximum likelihood using the unmarked
% package in R 4.1.2 (Fiske & Chandler, 2011; R Core Team 2021).
